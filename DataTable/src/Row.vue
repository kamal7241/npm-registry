<template>
  <tr
    :data-id="rowId"
    ref="vbt_row"
    :style="{background: (rowHiglighted) ? highlightRowHoverColor : 0}"
    :class="rowClasses"
...
export default {
  ...
  methods: {
    addRow(shiftKey) {
      this.$emit('add-row', { shiftKey, rowIndex: this.rowIndex });
    },
    removeRow(shiftKey) {
      this.$emit('remove-row', { shiftKey, rowIndex: this.rowIndex });
    },
    ...
    cellClasses(column) {
      ...
      } if (typeof this.propCellClasses === 'object') {
  ...
  computed: {
    ...
    userRowClasses() {
      ...
      } if (typeof this.propRowClasses === 'object') {
