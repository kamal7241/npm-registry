        ...
        <div class="col-md-9 col-12" v-if="dataType=='text'">
          <div class="form-group">
            ...
            <textarea
              ...
              @focus="error= false"
              rows="8"
              @oninput="this.isArabic(this.reason)"
            ></textarea>

            <span
              v-if="this.isArabic(this.reason)"
              class="textarea-counter"
            >{{ maxReasonTextlength - reason.length + dir.counterLbl}}</span>
            ...
            <table class="table">
              ...
              <tbody>
                <tr :key="index" v-for="(data,index) in newDataSource">
                  ...
                  <td
                    v-if="(status=='Add'&& haveDate) "
                  >{{status!='Add' ?data.reasonDates[index] :data.reasonDates}}</td>
      ...
      <div class="row" v-if="status!='View' && status!='Preview'">
        <div class="col-md-12">
          <div class="custom-control custom-checkbox">
            ...
            >{{this.dir ? this.dir.undertakeLbl : ""}}</label>
          </div>
        </div>
      </div>
      <div class="row">
        ...
        >{{dir ? dir.maxSizeLbl : ""}} {{maxSize}}</v-alert>
      </div>
    </div>
  </div>
</template>
